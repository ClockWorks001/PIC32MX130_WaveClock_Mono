...
#define SELECT()	LATBbits.LATB14 = 0	/* CS = L */
#define	DESELECT()	LATBbits.LATB14 = 1	/* CS = H */
//#define	MMC_SEL		!LATBbits.LATB14		/* CS status (true:CS == L) */
#define	MMC_SEL		!PORTBbits.RB14		/* CS status (true:CS == L) */
...
void init_spi (void)		/* Initialize SPI port */
{
	...
	SpiChnOpen(SPI_CHN, SPI_OPEN, 2);	// divide fpb by 2,4 , configure the I/O ports. Not using SS in this example
...
void xmit_spi (BYTE d)		/* Send a byte to the MMC */
{
	...
	SPI2BUF = (unsigned long)d;
	while (SPI2STATbits.SPIBUSY) ;
	{
		iWait++;
		if(iWait > 100000) return ; //ERROR99;
	};
	TEST_BYTE = (BYTE)SPI2BUF;
}
...
BYTE rcv_spi (void)			/* Send a 0xFF to the MMC and get the received byte */
{
	...
	{
		iWait++;
		if(iWait > 100000) return 0XFF ; //ERROR99;
	};
