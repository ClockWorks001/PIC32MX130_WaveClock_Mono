...
void vTxFifo (void)
{
	...
	while(!U2STAbits.UTXBF){	// TX buffer has space //
		if (TxFifo.ct) {		// If any data is available, pop a byte and send it. //
			...
			//UxA/B have "sticky" interrupts, as you  have seen. !!!
			// ==>http://www.microchip.com/forums/m563689.aspx
			// ==>https://forum.sparkfun.com/viewtopic.php?f=6&t=27051
			INTEnable(INT_SOURCE_UART_TX(UART2), INT_DISABLED);
			break;
		}
	}
...
void uart_putc (BYTE d)
{
	...
	//if (!U2STAbits.UTXBF) {		// If TX buffer is not full.(UTXBF=0) //
		//TxRun = 1;
		//IFS1bits.U2TXIF = 1;	/* Force trigger Tx interrupt */
	//	vTxFifo();
	//}
	//IEC1bits.U2TXIE = 1; // _EI();
